...
import JuliaupDistributions
const Juliaup = JuliaupDistributions
...
function stub_tarball(destination, root_name, version)
    ...
    # A single top-level directory, which is what juliaup's strip of one path
    # component expects.
    run(pipeline(`tar -C $staging -czf $destination $root_name`))
...
function distribution_for(dist, channel)
    return Juliaup.Distribution(; channel,
